...
export function getRecentHistory(uid, count) {
  if (!uid) return Promise.resolve([]);
  return fs().then(({ db, collection, query, orderBy, limit, getDocs }) => {
...
export function countHistory(uid, type) {
  ...
  return fs().then(({ db, collection, query, where, getCountFromServer }) => {
...
export function clearHistory(uid) {
  if (!uid) return Promise.resolve();
  return fs().then(({ db, collection, query, getDocs, deleteDoc }) => {
